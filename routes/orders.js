...
router.get('/gb', async (req, res) => {
  const order = await Order.find().sort('placedTime');
  res.send(order);
});

router.post('/gb', [auth, permit('buyer', 'admin')],  async (req, res) => {
...
router.put('/gb/:id', [auth, permit('buyer', 'admin')], async (req, res) => {
...
router.delete('/gb/:id', [auth, permit('admin')], async (req, res) => {
  const order = await Order.findByIdAndRemove(req.params.id);
...
router.get('/gb/id/:id', [auth], async (req, res) => {
  const order = await Order.findById(req.params.id);
